...
//	BasicDBObject query = new BasicDBObject("NPI", );
	MongoClient client = new MongoClient("192.168.0.151",27081);
			//(new ServerAddress("192.168.0.151", 27081));

	MongoDatabase DB = client.getDatabase("Provider");
	MongoCollection<Document> notifyColl = DB.getCollection("LoopStats");
//	BasicDBObject query = new BasicDBObject("NPI", );
	FindIterable<Document> search = notifyColl.find();
	Iterator<Document> Iter = search.iterator();
	while(Iter.hasNext()){
		Document doc = Iter.next();
		count = doc.getLong("Updated Documents");
	}
...
<div class="column col-lg-2 col-md-2 col-sm-2" style="height:80px; margin-left:330px; ">
